...
<body>
  ...
  <!-- <div class="left-section">
    ...
    <div>
      <h1><i class="fa-solid fa-comments"></i> Fórum</h1>
    </div>
    <div>
      <h1><i class="fa-solid fa-hand-holding-hand"></i> ONGS</h1>
    </div>
  </div> -->
  <div class="right-section">
    <div class="ongs-sugestion">
    <h1 class="section-title">Conheça as ONGS</h1>
    <?php
        $stmt = $bd -> prepare("SELECT * FROM ongs ORDER BY rand() LIMIT 5;");
        $stmt -> execute();
        $queryRandOngs = $stmt -> fetch(PDO::FETCH_ASSOC);
    if($queryRandOngs > 0){
    while($rowRandOngs = $queryRandOngs = $stmt -> fetch(PDO::FETCH_ASSOC)){
      $imgRandOngs = $rowRandOngs['img-ref'];
      $nomeRandOngs = $rowRandOngs['nome'];
      $categoriaRandOngs = $rowRandOngs['categoria'];
    ?>
    <div class="user-display">
      <img src="../img/ongs/<?php echo $imgRandOngs; ?>" alt="">
      <div>
        <h1><?php echo $nomeRandOngs ?> <i class="fa-solid fa-circle-check fa-xs"></i></h1>
        <p><?php echo $categoriaRandOngs ?></p>
      </div>
    </div>
    <?php }} ?>
  </div>
    <div class="users-sugestion">
    <h1 class="section-title">Quem seguir</h1>
    <?php
        $stmt = $bd -> prepare("SELECT * FROM usuarios ORDER BY rand() LIMIT 5;");
        $stmt -> execute();
        $queryRandUsers = $stmt -> fetch(PDO::FETCH_ASSOC);
    if($queryRandUsers > 0){
    while($rowRandUsers = $queryRandUsers = $stmt -> fetch(PDO::FETCH_ASSOC)){
      $imgRandUser = $rowRandUsers['imgUser'];
      $nomeRandUser = $rowRandUsers['nome'];
      $sobreMimRandUser = $rowRandUsers['sobreMim'];
    ?>
    <div class="user-display">
      <img src="<?php echo $imgRandUser; ?>" alt="">
      <div>
        <h1><?php echo $nomeRandUser ?></h1>
        <p><?php echo $sobreMimRandUser ?></p>
      </div>
    </div>
    <?php }} ?>
  </div>
